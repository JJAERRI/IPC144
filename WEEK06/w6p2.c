...
int main(void)
{
	...
	{
		int year, month;
		int haveFinanceOption = 0;
		int MonthCal = total / income;
		year = (MonthCal + 1) / 12;
		month = (MonthCal + 1) % 12;
		int priorityFilter;
		double totalFIltercost = 0.0;
		...
		else if (select == 2)
		{
			...
			int MonthCal = totalFIltercost / income;
			year = (MonthCal + 1) / 12;
			month = (MonthCal + 1) % 12;
